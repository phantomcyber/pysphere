...
try: 
    ...
    raise RuntimeError('use python-2.4 or later, cannot set function names in python "%s"'\
        %sys.version)
del _x
...
class pyclass_type(type):
    ...
    def __new__(cls, classname, bases, classdict):
        ...
        if len([b for b in bases if not b == object]) > 0:
        ...
        if hasattr(typecode, 'attribute_typecode_dict'):
            ...
            for key,what in list(attribute_typecode_dict.items()):
    ...
    def __create_functions_from_what(what):
        if not isinstance(what, Callable):
            ...
            if what.maxOccurs == TC.UNBOUNDED or what.maxOccurs > 1:
                def set(self, value):
        ...
        set.__name__ = 'set_element_%s' %what.pname
